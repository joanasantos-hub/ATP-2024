        ...
        if autores_input:
            for linha in autores_input.split("\n"):
                ...
                if len(campos) >= 1:
                    ...
                    if len(campos) > 2 and campos[2]: # Não utilizamos um bloco elif neste caso, uma vez que irá afetar a lógica do processamento da informação do ORCID do autor
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, '-CANCELAR-'):
            ...
            if busca_title:
                ...
                publicação = logic.Atualizar_Pub(publicações, busca_title,'','','','') # Durante a procura da publicação que queremos atualizar não definimos logo o resto dos argumentos!
               
                if publicação:
                   
                    detalhes = ["=== Detalhes da Publicação ===\n"]
                    if publicação.get('title'):
                        detalhes.append(f"* title: {publicação['title']}\n")
                    if publicação.get('abstract'):
                        detalhes.append(f"* abstract: {publicação['abstract']}\n")
                    if publicação.get('keywords'):
                        detalhes.append(f"* keywords: {publicação['keywords']}\n")
                    ...
                    if publicação.get('publish_date'):
                        detalhes.append(f"* publish_date: {publicação['publish_date']}\n")
                    ...
                    window.extend_layout(window, [
                        [sg.Multiline(detalhes_texto, size=(90, 15), disabled=True, autoscroll=True, key= "-DETALHES_PUBLICACAO-")],
                        [sg.Text("Campo a alterar/Novo campo para a publicação:", size=(37, 1), key= '-TEXTO_CAMPO-'), sg.Input(key= "-ALTERAR_CHAVE-")],
                        [sg.Text("Informação do campo atualizada:", size=(25, 1), key= '-TEXTO_INFO-'), sg.Input(key= "-UPDATE_CHAVE-")],
                        [sg.Text("Nome do autor (para alterações específicas):", size=(37, 1)), sg.Input(key="-NOME_AUTOR-")],
            ...
            nome_autor = values.get("-NOME_AUTOR-", "").strip()
            update_autor = values["-UPDATE_AUTOR-"].strip()

            if alterar_chave and (update_chave or update_autor):
                logic.Atualizar_Pub(publicações, busca_title, alterar_chave, update_chave, nome_autor, update_autor)
...
def Gerar_Gráficos():
    ...
    layout_autor = [
        [sg.Text('Introduza o nome do autor:'), sg.InputText(key= '-NOME-')],
        [sg.Button('Gerar Relátorio para o Autor', key= '-RELAT_AUTOR-'),sg.Button('Cancelar', key= '-CANCELAR_RELAT_AUTOR-')]  
    ]

    layout_mês = [
        [sg.Text('Introduza o ano de publicação:'), sg.InputText(key= '-ANO-')],
        [sg.Button('Gerar Relátorio para o Ano', key= '-RELAT_MES-'),sg.Button('Cancelar', key= '-CANCELAR_RELAT_MES-')]  
    ]
    ...
    while stop is False:
        ...
        if event in (sg.WIN_CLOSED, '-CANCELAR-'):
            ...
            window.extend_layout(window, layout_mês)
        
        elif event == '-RELAT_MES-':

            x = values['-ANO-'].strip().lower()
            if x:
                dist_Mês = logic.Distribuição_Mês(publicações, x)
                gd.graf_Mês(dist_Mês)
            else:
                sg.popup("Por favor, insira um ano de publicação válido.")
        
        elif event == '-CANCELAR_RELAT_MES-':
            window.close()

        elif event == '-PUB_AUTOR-':
            window.extend_layout(window, layout_autor)

        elif event == '-RELAT_AUTOR-':

            autor_nome = values['-NOME-'].strip().lower()
            if autor_nome:
                dist_Autor = logic.Distribuição_Autor(publicações, autor_nome)
                gd.graf_Autor(dist_Autor)
            else:
                sg.popup("Por favor, insira um nome de autor válido.")
        
        elif event == '-CANCELAR_RELAT_AUTOR-':
            window.close()
...
def Gerar_Relatórios():
    ...
    while True:
        ...
        if event in (sg.WIN_CLOSED, '-CANCELAR-'):
            ...
            window.extend_layout(window, [[sg.Button('Fechar', key= '-CANCELAR-')]])
            ...
            x = values['-ANO-'].strip().lower()
            if x:
                resultados = logic.Distribuição_Mês(publicações, x)
                window['-RESULTADOS-'].update('\n'.join(f"{chave}: {valor}" for chave, valor in resultados.items()))
            else:
                sg.popup("Por favor, insira um ano de publicação válido.")
            ...
            autor_nome = values['-NOME-'].strip().lower()
            if autor_nome:
                resultados = logic.Distribuição_Autor(publicações, autor_nome)
                window['-RESULTADOS-'].update('\n'.join(f"{chave}: {valor}" for chave, valor in resultados.items()))
            else:
                sg.popup("Por favor, insira um nome de autor válido.")
